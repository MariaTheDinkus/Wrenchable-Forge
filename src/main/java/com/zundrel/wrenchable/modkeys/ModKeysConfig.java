...
public class ModKeysConfig {
	...
	private static File getFile() {
		final File configDir = FMLPaths.GAMEDIR.get().resolve(FMLConfig.defaultConfigPath()).toFile();
		if (!configDir.exists()) {
			WrenchableMain.LOGGER.warn("[Wrenchable ModKeys] Could not access configuration directory: " + configDir.getAbsolutePath());
		}
	...
	private static Properties getProperties(File configFile) {
		...
		if (configFile.exists()) {
			try (FileInputStream stream = new FileInputStream(configFile)) {
				...
				WrenchableMain.LOGGER.warn("[Wrenchable ModKeys] Could not read property file '" + configFile.getAbsolutePath() + "'", e);
			}
		}
	...
	private static void saveProperties(File configFile, Properties properties) {
		try (FileOutputStream stream = new FileOutputStream(configFile)) {
			properties.store(stream, "Wrenchable ModKeys properties file");
		} catch (final IOException e) {
			WrenchableMain.LOGGER.warn("[Wrenchable ModKeys] Could not store property file '" + configFile.getAbsolutePath() + "'", e);
		}
	}
